            ...
            while(true)
            {
                ...
                catch(IOException e)
                {
                    ...
                    if(isContinueOnError(e))
                    {
                         ...
                         */
                        Log.w(PDFBox.LOG_TAG, "Parsing Error, Skipping Object");
            ...
            if (peekedChar == 's')
            {
                ...
                if(!"%%EOF".equals(eof)) 
                {
                    ...
                    else 
                    {
                        ...
                        Log.w(PDFBox.LOG_TAG, "expected='%%EOF' actual='" + eof + "'");
        ...
        else
        {
            ...
            if( !endObjectKey.equals( "endobj" ) )
            {
                ...
                else if(endObjectKey.trim().endsWith("endobj"))
                {
                     ...
                     */
                    Log.w(PDFBox.LOG_TAG, "expected='endobj' actual='" + endObjectKey + "' ");
                }
    ...
    protected boolean parseXrefTable( long startByteOffset ) throws IOException
    {
        ...
        while(true)
        {
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                {
                    Log.w(PDFBox.LOG_TAG, "invalid xref line: " + currentLine);
                    break;
                }
        ...
        if (header.startsWith(PDF_HEADER))
        {
            if (!header.matches(PDF_HEADER + "\\d.\\d"))
            {
                ...
                if (header.length() < PDF_HEADER.length() + 3)
                {
                    ...
                    Log.d(PDFBox.LOG_TAG, "No pdf version found, set to " + PDF_DEFAULT_VERSION + " as default.");
                }
                else
                {
        ...
        else
        {
            if (!header.matches(FDF_HEADER + "\\d.\\d"))
            {
                if (header.length() < FDF_HEADER.length() + 3)
                {
                    ...
                    Log.d(PDFBox.LOG_TAG, "No fdf version found, set to " + FDF_DEFAULT_VERSION + " as default." );
                }
                else
                {
        ...
        private static void resolveConflicts(COSDocument document, List<ConflictObj> conflictList) throws IOException
        {
            ...
            if (conflicts.hasNext())
            {
                ...
                do
                {
                    ...
                    Long offset = Long.valueOf(o.offset);
                    if (tolerantConflicResolver(values, offset, 4))
                    {
                        ...
                        {
                            Log.v(PDFBox.LOG_TAG, "Conflict object ["+o.objectKey+"] at offset "+offset
