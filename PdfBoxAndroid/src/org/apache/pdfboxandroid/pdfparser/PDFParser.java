            ...
            while(true)
            {
                ...
                catch(IOException e)
                {
                    ...
                    if(isContinueOnError(e))
                    {
                         ...
                         */
                        Log.w("Parsing Error, Skipping Object", PDFBox.LOG_TAG);
    ...
    protected boolean parseXrefTable( long startByteOffset ) throws IOException
    {
        ...
        while(true)
        {
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                {
                    Log.w("invalid xref line: " + currentLine, PDFBox.LOG_TAG);
                    break;
                }
        ...
        if (header.startsWith(PDF_HEADER))
        {
            if (!header.matches(PDF_HEADER + "\\d.\\d"))
            {
                ...
                if (header.length() < PDF_HEADER.length() + 3)
                {
                    ...
                    Log.d("No pdf version found, set to " + PDF_DEFAULT_VERSION + " as default.", PDFBox.LOG_TAG);
                }
                else
                {
        ...
        else
        {
            if (!header.matches(FDF_HEADER + "\\d.\\d"))
            {
                if (header.length() < FDF_HEADER.length() + 3)
                {
                    ...
                    Log.d("No fdf version found, set to " + FDF_DEFAULT_VERSION + " as default." , PDFBox.LOG_TAG);
                }
                else
                {
            ...
            if (conflicts.hasNext())
            {
                ...
                do
                {
                    ...
                    if (tolerantConflicResolver(values, offset, 4))
                    {
                        ...
                        {
                            Log.v("Conflict object ["+o.objectKey+"] at offset "+offset
                                    +" found in the xref table, but the object numbers differ. Ignoring this object."
                                    + " The document is maybe malformed.", PDFBox.LOG_TAG);
                        }
                    }
                }
