...
class CloudyBorder
{
    ...
    void createCloudyPolygon(float[][] path) throws IOException
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (array.length == 2)
            {
                polygon[i] = new PointF(array[0], array[1]);
            }
            else if (array.length == 6)
            {
                // TODO Curve segments are not yet supported in cloudy border.
                polygon[i] = new PointF(array[4], array[5]);
            }
        }
        ...
        double right, double top, boolean isEllipse) throws IOException
    {
        ...
        if (w < 1.0)
        {
            polygon = new PointF[]
                {
                    new PointF((float)left, (float)bottom), new PointF((float)left, (float)top),
        ...
        else if (h < 1.0)
        {
            polygon = new PointF[]
                {
                    new PointF((float)left, (float)bottom), new PointF((float)right, (float)bottom),
                    new PointF((float)left, (float)bottom)
                };
        }
        else
        {
            polygon = new PointF[]
                {
                    new PointF((float)left, (float)bottom), new PointF((float)right, (float)bottom),
                    new PointF((float)right, (float)top), new PointF((float)left, (float)top),
                    new PointF((float)left, (float)bottom)
                };
        }
     ...
     */
    private void cloudyPolygonImpl(PointF[] vertices, boolean isEllipse)
        throws IOException
    {
        PointF[] polygon = removeZeroLengthSegments(vertices);
        ...
        int n0 = computeParamsPolygon(advIntermDefault, advCornerDefault, k, cloudRadius,
            Math.hypot(polygon[0].x - polygon[numPoints - 2].x, polygon[0].y - polygon[numPoints - 2].y), array);
        double alphaPrev = (n0 == 0) ? array[0] : ANGLE_34_DEG;

        for (int j = 0; j + 1 < numPoints; j++)
        {
            PointF pt = polygon[j];
            PointF ptNext = polygon[j + 1];
            double length = Math.hypot(ptNext.x - pt.x, ptNext.y - pt.y);
            ...
            if (j == 0)
            {
                PointF ptPrev = polygon[numPoints - 2];
            ...
            PointF[] template = getIntermediateCurlTemplate(angleCur, cloudRadius);
     ...
     */
    private PointF[] getIntermediateCurlTemplate(double angleCur, double r)
        throws IOException
    {
     ...
     */
    private void outputCurlTemplate(PointF[] template, double x, double y)
        throws IOException
    {
        ...
        {
            PointF a = template[i];
            PointF b = template[i + 1];
            PointF c = template[i + 2];
    ...
    private void reversePolygon(PointF[] points)
    {
     ...
     */
    private void getPositivePolygon(PointF[] points)
    {
     ...
     */
    private double getPolygonDirection(PointF[] points)
    {
        double a = 0;
        int len = points.length;
        for (int i = 0; i < len; i++)
    ...
    private void getArc(double startAng, double endAng, double rx, double ry,
        double cx, double cy, ArrayList<PointF> out, boolean addMoveTo) throws IOException
    {
        ...
        if (addMoveTo)
        {
            if (out != null)
            {
                out.add(new PointF((float)startx, (float)starty));
            }
            else
            {
    ...
    private void getArcSegment(double startAng, double endAng, double cx, double cy,
        double rx, double ry, ArrayList<PointF> out, boolean addMoveTo) throws IOException
    {
        ...
        if (Double.compare(denom, 0.0) == 0)
        {
            ...
            if (addMoveTo)
            {
                ...
                if (out != null)
                {
                    out.add(new PointF((float)xs, (float)ys));
                }
                else
                {
                    moveTo(xs, ys);
        ...
        if (out != null)
        {
            out.add(new PointF((float)p1x, (float)p1y));
            out.add(new PointF((float)p2x, (float)p2y));
            out.add(new PointF((float)p3x, (float)p3y));
        }
        else
        {
     ...
     */
    private static PointF[] flattenEllipse(double left, double bottom,
        double right, double top)
    {
        return new PointF[0]; // TODO: PdfBox-Android
    }
        ...
        final double rightOrig, final double topOrig) throws IOException
    {
        ...
        PointF[] flatPolygon = flattenEllipse(left, bottom, right, top);
        ...
        double totLen = 0;
        for(int i = 1; i < numPoints; i++){
            totLen += Math.hypot(flatPolygon[i].x - flatPolygon[i - 1].x, flatPolygon[i].y - flatPolygon[i - 1].y);
        }
        ...
        PointF[] centerPoints = new PointF[centerPointsLength];
        ...
        {
            PointF p1 = flatPolygon[i];
            PointF p2 = flatPolygon[i + 1];
            double dx = p2.x - p1.x;
            double dy = p2.y - p1.y;
            ...
            if (lengthTodo >= curlAdvance - comparisonToler || i == numPoints - 2)
            {
                ...
                do
                {
                    ...
                    {
                        centerPoints[centerPointsIndex++] = new PointF((float)x, (float)y);
                    }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            ...
            PointF pt = centerPoints[i];
            PointF ptNext = centerPoints[idxNext];

            if (i == 0)
            {
                PointF ptPrev = centerPoints[numPoints - 1];
                anglePrev = Math.atan2(pt.y - ptPrev.y, pt.x - ptPrev.x);
     ...
     */
    private double computeParamsEllipse(PointF pt, PointF ptNext,
    ...
    private PointF[] removeZeroLengthSegments(PointF[] polygon)
    {
        ...
        PointF[] polygonNew = new PointF[npNew];
        int j = 0;
        for (int i = 0; i < np; i++)
        {
            PointF pt = polygon[i];
    ...
    private void moveTo(PointF p) throws IOException
    {
        moveTo(p.x, p.y);
    }
    ...
    private void lineTo(PointF p) throws IOException
    {
        lineTo(p.x, p.y);
    }
